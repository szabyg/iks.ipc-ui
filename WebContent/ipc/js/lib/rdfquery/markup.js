$(document).ready(function () {
  var 
    ...
    ontology = {
      ...
      '<http://rdf.data-vocabulary.org/#contact>': {
              type: 'property',
              aliases: ['contact']
            },
      '<http://rdf.data-vocabulary.org/#contact>': {
          type: 'property',
          range: '<http://rdf.data-vocabulary.org/#Person>',
          aliases: ['contact']
        }
    },
    ...
    populateLists = function () {
      ...
      rdf
	      .prefix('rdfs', ns.rdfs)
	      .prefix('v', ns.v)
	      .where('?person a v:Person')
	      .where('?person v:name ?label')
	      .each(function () {
	        addIndividual(people, this.person, this.label.value);
	      })
	      .reset()
	      .where('?place a v:Event')
	      .where('?place v:summary ?label')
	      .each(function () {
	        addIndividual(events, this.place, this.label.value);
	      })
	      .reset()
	      .where('?place a v:Address')
	      .where('?place v:locality ?label')
	      .each(function () {
	        addIndividual(places, this.place, this.label.value);
	      });
    };
  
  $('#people h2, #places h2, #events h2')
    .bind('click', function () {
